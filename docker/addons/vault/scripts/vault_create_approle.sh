...
vaultCreatePolicy() {
    ...
    if is_container_running "magistrala-vault"; then
        docker cp "$scriptdir/magistrala_clients_certs_issue.hcl" magistrala-vault:/vault/magistrala_clients_certs_issue.hcl
        vault policy write -namespace=${MG_VAULT_NAMESPACE} -address=${MG_VAULT_ADDR} magistrala_clients_certs_issue /vault/magistrala_clients_certs_issue.hcl
    else
        vault policy write -namespace=${MG_VAULT_NAMESPACE} -address=${MG_VAULT_ADDR} magistrala_clients_certs_issue "$scriptdir/magistrala_clients_certs_issue.hcl"
    fi
}
...
vaultDeleteRole() {
    ...
    vault delete -namespace=${MG_VAULT_NAMESPACE} -address=${MG_VAULT_ADDR} auth/approle/role/magistrala_clients_certs_issuer
}

vaultCreateRole() {
    echo "Creating new AppRole"
    vault write -namespace=${MG_VAULT_NAMESPACE} -address=${MG_VAULT_ADDR} auth/approle/role/magistrala_clients_certs_issuer \
    token_policies=magistrala_clients_certs_issue secret_id_num_uses=0 \
