...
func TestCreateThings(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc: "create a new thing with missing identity",
			thing: mgclients.Client{
	...
	for _, tc := range cases {
		...
		repoCall1 := auth.On("AddPolicies", mock.Anything, mock.Anything).Return(tc.addPolicyResponse, tc.addPolicyErr)
		repoCall2 := auth.On("Authorize", mock.Anything, mock.Anything).Return(tc.authResponse, nil)
		repoCall3 := cRepo.On("Save", context.Background(), mock.Anything).Return([]mgclients.Client{tc.thing}, tc.err)
...
func TestListClients(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc:  "list all clients successfully as admin",
			token: validToken,
			id:    adminID,
			page: mgclients.Page{
				Offset:    0,
			...
			identifyResponse:  &magistrala.IdentityRes{Id: adminID, UserId: adminID, DomainId: domainID},
			authorizeResponse: &magistrala.AuthorizeRes{Authorized: true},
			retrieveAllResponse: mgclients.ClientsPage{
				Page: mgclients.Page{
					Total:  2,
		...
		{
			desc: "list all clients as admin with invalid token",
			page: mgclients.Page{
				Offset:    0,
				Limit:     100,
				ListPerms: true,
		...
		{
			desc: "list all clients as admin with invalid id",
			id:   wrongID,
			page: mgclients.Page{
				Offset:    0,
				Limit:     100,
			...
			token:             validToken,
			identifyResponse:  &magistrala.IdentityRes{Id: wrongID, UserId: adminID, DomainId: domainID},
			authorizeResponse: &magistrala.AuthorizeRes{Authorized: false},
			authorizeErr:      svcerr.ErrAuthorization,
			err:               svcerr.ErrAuthorization,
		},
		{
			desc: "list all clients as admin with empty domain id",
			id:   adminID,
			page: mgclients.Page{
				Offset:    0,
				Limit:     100,
			...
			token:            validToken,
			identifyResponse: &magistrala.IdentityRes{Id: adminID, UserId: adminID, DomainId: ""},
		...
		{
			desc:  "list all clients as non admin successfully",
			id:    nonAdminID,
			token: validToken,
			page: mgclients.Page{
				Offset:    0,
		...
		{
			desc:  "list all clients as non admin with unauthorized user",
			id:    nonAdminID,
			token: validToken,
			page: mgclients.Page{
				Offset:    0,
	...
	for _, tc := range cases {
		...
		repoCall3 := cRepo.On("RetrieveAllByIDs", context.Background(), mock.Anything).Return(tc.retrieveAllResponse, tc.retrieveAllErr)
...
func TestListMembers(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc:    "list clients for an owner with permissions",
			token:   validToken,
			groupID: testsutil.GenerateUUID(t),
			page: mgclients.Page{
				Owner:     adminEmail,
		...
		{
			desc:    "list permissiond with unauthorized user",
			token:   validToken,
			groupID: testsutil.GenerateUUID(t),
			page: mgclients.Page{
				Owner:     adminEmail,
		...
		{
			desc:    "list permissiond with failed to list objects",
			token:   validToken,
			groupID: testsutil.GenerateUUID(t),
			page: mgclients.Page{
				Owner:     adminEmail,
	...
	for _, tc := range cases {
		...
		repoCall4 := auth.On("ListPermissions", mock.Anything, mock.Anything).Return(&magistrala.ListPermissionsRes{Permissions: tc.permissions}, nil)
...
func TestDeleteClient(t *testing.T) {
	...
	for _, tc := range cases {
		...
		repoCall3 := cache.On("Remove", mock.Anything, tc.clientID).Return(tc.removeErr)
