...
BUILD_DIR ?= build
SERVICES = bootstrap certs postgres-writer postgres-reader timescale-writer timescale-reader fluxmq
CLI = cli
...
# auth/broker chain behind it depend on atom itself being reachable. A
# single `docker compose up` for
...
# so atom boots cleanly and fluxmq-auth and the FluxMQ nodes behind it can
# all finish starting. Targeting nginx (not just fluxmq-auth) matters: the
# regular `up` that follows recreates atom with the real value, and it needs
# the AMQP proxy nginx fronts to already be genuinely functional at that
# point, not merely "started", or the same deadlock reopens inside the
# second invocation. Targeting nginx pulls in the full chain (atom-db ->
# atom -> fluxmq-auth -> fluxmq-node1/2/3 -> nginx) via the compose file's own depends_on
