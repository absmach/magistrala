...
import (
	...
	domainsapi "github.com/absmach/magistrala/domains/api/http"
...
func TestListDomains(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc:  "list domains with single tag",
			token: validToken,
			page: domains.Page{
				Offset: 0,
				Limit:  10,
		...
		{
			desc:  "list domains with multiple tags and OR operator",
			token: validToken,
			page: domains.Page{
				Offset: 0,
				Limit:  10,
				Order:  api.DefOrder,
				Dir:    api.DefDir,
				Tags:   domains.TagsQuery{Elements: []string{"tag1", "tag2", "tag3"}, Operator: domains.OrOp},
			},
			listDomainsResp: domains.DomainsPage{
				Total:   1,
				Domains: []domains.Domain{domain},
		...
		{
			desc:  "list domains with multiple tags and AND operator",
			token: validToken,
			page: domains.Page{
				Offset: 0,
				Limit:  10,
				Order:  api.DefOrder,
				Dir:    api.DefDir,
				Tags:   domains.TagsQuery{Elements: []string{"tag1", "tag2", "tag3"}, Operator: domains.AndOp},
			},
			listDomainsResp: domains.DomainsPage{
				Total:   1,
				Domains: []domains.Domain{domain},
