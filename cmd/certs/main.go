...
func main() {
	...
	if cfg.PkiHost == "" {
		logger.Error(ctx, "No host specified for PKI engine")
		exitCode = 1
		return
	}
	...
	if err != nil {
		logger.Error(ctx, "failed to configure client for PKI engine")
		exitCode = 1
		return
	}
	...
	if err := g.Wait(); err != nil {
		logger.Error(ctx, fmt.Sprintf("Certs service terminated: %s", err))
	}
}
