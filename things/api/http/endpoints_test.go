...
func TestListClientsByGroup(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc:    "list clients by group with offset",
			token:   validToken,
			query:   "&offset=1",
			groupID: client.ID,
			listClientsResponse: mgclients.ClientsPage{
		...
		{
			desc:    "list clients by group with invalid offset",
			token:   validToken,
			query:   "&offset=invalid",
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with invalid limit",
			token:   validToken,
			query:   "&limit=invalid",
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with limit greater than 100",
			token:   validToken,
			query:   fmt.Sprintf("&limit=%d", api.MaxLimitSize+1),
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with channel_id",
			token:   validToken,
		...
		{
			desc:    "list clients by group with invalid channel_id",
			token:   validToken,
		...
		{
			desc:    "list clients by group with duplicate channel_id",
			token:   validToken,
		...
		{
			desc:    "list clients by group with connected set",
			token:   validToken,
		...
		{
			desc:    "list clients by group with invalid connected set",
			token:   validToken,
		...
		{
			desc:   "list clients by group with duplicate connected set",
			token:  validToken,
		...
		{
			desc:    "list clients by group with invalid group id",
			token:   validToken,
			query:   "",
			groupID: "invalid",
			status:  http.StatusForbidden,
		...
		{
			desc:    "list clients by group with empty group id",
			token:   validToken,
			query:   "",
			groupID: "",
			status:  http.StatusBadRequest,
		...
		{
			desc:  "list clients by group with status",
		...
		{
			desc:    "list clients by group with invalid status",
			query:   "&status=invalid",
			token:   validToken,
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with duplicate status",
		...
		{
			desc:  "list clients by group with metadata",
			token: validToken,
			listClientsResponse: mgclients.ClientsPage{
				Page: mgclients.Page{
					Total: 1,
		...
		{
			desc:    "list clients by group with invalid metadata",
			query:   "&metadata=invalid",
			groupID: client.ID,
			token:   validToken,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with duplicate metadata",
			query:   "&metadata=%7B%22domain%22%3A%20%22example.com%22%7D&metadata=%7B%22domain%22%3A%20%22example.com%22%7D",
			groupID: client.ID,
			token:   validToken,
			status:  http.StatusBadRequest,
		...
		{
			desc:  "list clients by group with permission",
		...
		{
			desc:    "list clients by group with invalid permission",
			query:   "&permission=invalid",
			token:   validToken,
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with duplicate permission",
		...
		{
			desc:  "list clients by group with list permission",
			query: "&list_perms=true",
			token: validToken,
			listClientsResponse: mgclients.ClientsPage{
				Page: mgclients.Page{
		...
		{
			desc:    "list clients by group with invalid list permission",
			query:   "&list_perms=invalid",
			token:   validToken,
			groupID: client.ID,
			status:  http.StatusBadRequest,
		...
		{
			desc:    "list clients by group with duplicate list permission",
		...
		{
			desc:    "list clients by group with all query params",
	...
	for _, tc := range cases {
		req := testRequest{
			client:      ts.Client(),
			method:      http.MethodGet,
			url:         ts.URL + fmt.Sprintf("/things?channel=%s", tc.groupID) + tc.query,
			contentType: contentType,
			token:       tc.token,
		}
		svcCall := svc.On("ListClients", mock.Anything, tc.token, mock.Anything).Return(tc.listClientsResponse, tc.err)
		res, err := req.make()
		assert.Nil(t, err, fmt.Sprintf("%s: unexpected error %s", tc.desc, err))
