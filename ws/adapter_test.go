...
func newService() (ws.Service, *mocks.PubSub, *thmocks.AuthzClient) {
	pubsub := new(mocks.PubSub)
	tauth := new(thmocks.AuthzClient)

	return ws.New(tauth, pubsub), pubsub, tauth
}

func TestSubscribe(t *testing.T) {
	svc, pubsub, tauth := newService()
	...
	for _, tc := range cases {
		...
		repocall1 := tauth.On("Authorize", mock.Anything, mock.Anything).Return(&magistrala.AuthorizeRes{Authorized: true, Id: thingID}, nil)
