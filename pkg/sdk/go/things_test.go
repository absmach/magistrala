...
func TestSearchThings(t *testing.T) {
	...
	cases := []struct {
		...
		searchRes    mgclients.ClientsPage
		...
		{
			desc:         "search things with authorized token",
			token:        validToken,
			identifyRes:  &magistrala.IdentityRes{Id: validID, DomainId: testsutil.GenerateUUID(t)},
		...
		{
			desc:  "search for things with invalid length of query",
			token: validToken,
			page: sdk.PageMetadata{
				Offset: offset,
				Limit:  limit,
		...
		{
			desc:  "search for things with no domain id",
			token: validToken,
			page: sdk.PageMetadata{
				Offset: offset,
				Limit:  limit,
			...
			err:          errors.NewSDKErrorWithStatus(svcerr.ErrDomainAuthorization, http.StatusForbidden),
		},
	}
	for _, tc := range cases {
		repoCall := auth.On("Identify", mock.Anything, &magistrala.IdentityReq{Token: tc.token}).Return(tc.identifyRes, tc.identifyErr)
		repoCall1 := auth.On("Authorize", mock.Anything, mock.Anything).Return(tc.authorizeRes, tc.authorizeErr)
		repoCall2 := cRepo.On("SearchBasicInfo", mock.Anything, mock.Anything).Return(tc.searchRes, tc.err)
		page, err := mgsdk.SearchThings(tc.page, tc.token)
		assert.Equal(t, tc.err, err, fmt.Sprintf("%s: expected error %s, got %s", tc.desc, tc.err, err))
		assert.Equal(t, tc.response, page.Things, fmt.Sprintf("%s: expected %v got %v\n", tc.desc, tc.response, page))
		repoCall.Unset()
		repoCall1.Unset()
