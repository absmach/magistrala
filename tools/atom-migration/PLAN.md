...
|-----------|---------|----------------|
| `workspaces-db` | `workspaces` | `workspaces`, `invitations`, `workspaces_roles`, `workspaces_role_actions`, `workspaces_role_members` |
...
| `auth-db` | `auth` | `pats`, `pat_scopes` (skip `keys` — short-lived JWTs; skip legacy `policies`/`workspaces` mirror) |
...
`...0006` workspace-creator role, `...0007/8/9` permission blocks.

---

## 3. Entity mapping

### 3.1 workspaces → tenants
| Magistrala `workspaces` | Atom `tenants` |
|---|---|
| `id` | `id` (preserved) |
| `name` | `name` |
...
|---|---|
| `id` | `entities.id` (preserved), `kind='human'`, `tenant_id=NULL` (users are global; workspace membership handled in §4), `profile_id` / `profile_version_id` = seeded active `user` profile |
...
|---|---|
| `id` | `entities.id` (preserved), `kind='device'`, `tenant_id=workspace_id`, `profile_id` / `profile_version_id` = seeded active `device` profile, falling back to legacy `client` profile |
...
|---|---|
| `id` | `id` (preserved), `kind='channel'`, `tenant_id=workspace_id` |
| `name` | `name` |
...
Rules-engine rules and report configs are workspace-scoped configuration objects
...
into `attributes` (JSONB). `tenant_id=workspace_id`; rows whose workspace has no
...
Magistrala groups organize clients/channels within a workspace (hierarchical,
...
families: `workspaces`, `clients`, `channels`, `groups`, `rules`, and `reports`.

Magistrala role shape: each role row is bound to a specific object instance
(`entity_id` = the workspace/client/channel/group id), has a set of action strings,
and a set of member ids (users).

Mapping per Magistrala role row → Atom:
1. **`roles`** row (preserve `id`, `name`, `tenant_id` = owning workspace when known).
2. **`permission_blocks`** row scoped to the object:
   - workspace roles → `scope_mode='tenant'`, `tenant_id=entity_id`
...
6. For **workspace** role members, also insert **`tenant_memberships`**
...
### Invitations
`workspaces.invitations` → `tenant_invitations` (preserve workspace_id, invitee_user_id,
...
(`workspace_id, entity_type, operation, entity_id`) for reference / future policy
reconstruction.
...
1. **Tenant alias** (workspace.route): lowercase-fold; must match slug regex and **not**
...
3. **Per-tenant name uniqueness**: `entities(name, tenant_id)` and
   `resources` names — Magistrala already enforces `(workspace_id, name)` so this is
   usually safe; still verify (users have no tenant, so global human-name dupes are
   fine because humans are keyed by id/email).
4. **FK integrity**: skip rows whose `workspace_id` has no surviving tenant; skip
...
tools/atom-migration/
  ...
  source/            // one reader per source DB (sqlx/pgx), returns typed structs
    workspaces.go users.go clients.go channels.go groups.go auth.go
...
- **Connectivity:** run the migrator as a one-shot container on
  `magistrala-base-net` (compose `run --rm`) so it resolves `workspaces-db`,
...
1. `docker compose stop` Magistrala **app** services (users, clients, channels,
   groups, workspaces, auth, adapters) — keep the `*-db` containers running.
...
7. Point remaining Magistrala services (messaging/bootstrap/certs/readers) at Atom
   for authn/authz; decommission SpiceDB + users/clients/channels/groups/workspaces/auth.
...
- Spot `POST /authz/check` for a sample of (user, workspace, action) and
