...
func main() {
	...
	if err := env.ParseWithOptions(&coapServerConfig, env.Options{Prefix: envPrefix}); err != nil {
		logger.Error(ctx, fmt.Sprintf("failed to load %s CoAP server configuration : %s", svcName, err))
		exitCode = 1
		return
	}
	...
	if err := g.Wait(); err != nil {
		logger.Error(ctx, fmt.Sprintf("CoAP adapter service terminated: %s", err))
	}
}
