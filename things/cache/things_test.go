...
func TestSave(t *testing.T) {
	...
	tscache := cache.NewCache(redisClient, 10*time.Minute)
...
func TestRemove(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc: "Remove thing with empty key from cache",
			key:  "",
			id:   "testID2",
			err:  nil,
		},
