...
// RetrieveAll provides a mock function with given fields: ctx, domainID, filter, offset, limit
func (_m *ConfigRepository) RetrieveAll(ctx context.Context, domainID string, filter bootstrap.Filter, offset uint64, limit uint64) bootstrap.ConfigsPage {
	ret := _m.Called(ctx, domainID, filter, offset, limit)
	...
	if rf, ok := ret.Get(0).(func(context.Context, string, bootstrap.Filter, uint64, uint64) bootstrap.ConfigsPage); ok {
		r0 = rf(ctx, domainID, filter, offset, limit)
	} else {
