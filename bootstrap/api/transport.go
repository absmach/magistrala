...
func MakeHandler(svc bootstrap.Service, authnMW smqauthn.AuthNMiddleware, reader bootstrap.ConfigReader, logger *slog.Logger, instanceID string) http.Handler {
	...
	r.Route("/{domainID}/clients", func(r chi.Router) {
		...
		r.With(authnMW.Middleware()).Put("/state/{clientID}", otelhttp.NewHandler(kithttp.NewServer(
			stateEndpoint(svc),
			decodeStateRequest,
			api.EncodeResponse,
