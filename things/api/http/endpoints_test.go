...
func TestListThings(t *testing.T) {
	...
	for _, tc := range cases {
		...
		repoCall := svc.On("ListClients", mock.Anything, tc.token, mock.Anything, mock.Anything).Return(mgclients.ClientsPage{}, tc.err)
		res, err := req.make()
		assert.Nil(t, err, fmt.Sprintf("%s: unexpected error %s", tc.desc, err))
...
func TestListMembers(t *testing.T) {
	...
	for _, tc := range cases {
		...
		repoCall := svc.On("ListClientsByGroup", mock.Anything, tc.token, mock.Anything, mock.Anything).Return(mgclients.MembersPage{}, tc.err)
		res, err := req.make()
		assert.Nil(t, err, fmt.Sprintf("%s: unexpected error %s", tc.desc, err))
