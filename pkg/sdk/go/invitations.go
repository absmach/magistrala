...
func (sdk mgSDK) SendInvitation(invitation Invitation, token string) (err error) {
	...
	url := sdk.invitationsURL + "/" + domainsEndpoint + "/" + invitation.DomainID + "/" + invitationsEndpoint
...
func (sdk mgSDK) Invitations(pm PageMetadata, token string) (invitations InvitationPage, err error) {
	endpoint := fmt.Sprintf("%s/%s/%s", domainsEndpoint, pm.DomainID, invitationsEndpoint)
...
func (sdk mgSDK) AcceptInvitation(domainID, token string) (err error) {
	url := sdk.invitationsURL + "/" + domainsEndpoint + "/" + domainID + "/" + invitationsEndpoint + "/" + acceptEndpoint
...
func (sdk mgSDK) RejectInvitation(domainID, token string) (err error) {
	url := sdk.invitationsURL + "/" + domainsEndpoint + "/" + domainID + "/" + invitationsEndpoint + "/" + rejectEndpoint
