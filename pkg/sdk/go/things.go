...
func (sdk mgSDK) CreateThing(thing Thing, domainID, token string) (Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint)
...
func (sdk mgSDK) CreateThings(things []Thing, domainID, token string) ([]Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, "bulk")
...
func (sdk mgSDK) Thing(id, domainID, token string) (Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, id)
...
func (sdk mgSDK) ThingPermissions(id, domainID, token string) (Thing, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, id, permissionsEndpoint)
...
func (sdk mgSDK) UpdateThing(t Thing, domainID, token string) (Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, t.ID)
...
func (sdk mgSDK) UpdateThingTags(t Thing, domainID, token string) (Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/tags", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, t.ID)
...
func (sdk mgSDK) UpdateThingSecret(id, secret, domainID, token string) (Thing, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/secret", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, id)
...
func (sdk mgSDK) changeThingStatus(id, status, domainID, token string) (Thing, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, id, status)
...
func (sdk mgSDK) ShareThing(thingID string, req UsersRelationRequest, domainID, token string) errors.SDKError {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, thingID, shareEndpoint)
...
func (sdk mgSDK) UnshareThing(thingID string, req UsersRelationRequest, domainID, token string) errors.SDKError {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.thingsURL, domainsEndpoint, domainID, thingsEndpoint, thingID, unshareEndpoint)
...
func (sdk mgSDK) ListThingUsers(thingID string, pm PageMetadata, token string) (UsersPage, errors.SDKError) {
	url, err := sdk.withQueryParams(sdk.usersURL, fmt.Sprintf("%s/%s/%s/%s/%s", domainsEndpoint, pm.DomainID, thingsEndpoint, thingID, usersEndpoint), pm)
	if err != nil {
