...
func setupChannels() (*httptest.Server, *gmocks.Service, *authmocks.AuthClient) {
	...
	thapi.MakeHandler(tsvc, gsvc, authClient, mux, logger, "")
	usapi.MakeHandler(usvc, authClient, true, gsvc, mux, logger, "", passRegex, provider)
	return httptest.NewServer(mux), gsvc, authClient
}
...
func TestCreateChannel(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			resp, err := mgsdk.CreateChannel(tc.channelReq, tc.token)
