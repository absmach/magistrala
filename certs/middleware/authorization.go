...
func (am *authorizationMiddleware) RenewCert(ctx context.Context, session authn.Session, serialNumber string) (crt.Certificate, error) {
	if err := am.checkUserWorkspacePermission(ctx, session, policies.MembershipPermission); err != nil {
...
func (am *authorizationMiddleware) RevokeBySerial(ctx context.Context, session authn.Session, serialNumber string) error {
	if err := am.checkUserWorkspacePermission(ctx, session, policies.AdminPermission); err != nil {
		return err
	}
...
func (am *authorizationMiddleware) checkUserWorkspacePermission(ctx context.Context, session authn.Session, permission string) error {
	req := authz.PolicyReq{
		Workspace:   session.WorkspaceID,
		SubjectType: policies.UserType,
		SubjectKind: policies.UsersKind,
		Subject:     session.WorkspaceUserID,
		Permission:  permission,
		ObjectType:  policies.WorkspaceType,
		Object:      session.WorkspaceID,
	}
