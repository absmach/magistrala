...
func TestReadAll(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0&limit=10", ts.URL, domainID, chanID),
			token:        userToken,
			authResponse: true,
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=-1&limit=10", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0&limit=0", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=abc&limit=10", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0&limit=abc", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels//messages?offset=0&limit=10", ts.URL, domainID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0&offset=1&limit=10", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0&limit=20&limit=10", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?limit=10", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?offset=0", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?format=messages", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?subtopic=%s&protocol=%s", ts.URL, domainID, chanID, subtopic, httpProt),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?publisher=%s", ts.URL, domainID, chanID, pubID2),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
...

		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?protocol=http", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?name=%s", ts.URL, domainID, chanID, msgName),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f", ts.URL, domainID, chanID, v),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=%s", ts.URL, domainID, chanID, v, readers.EqualKey),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=%s", ts.URL, domainID, chanID, v+1, readers.LowerThanKey),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=%s", ts.URL, domainID, chanID, v+1, readers.LowerThanEqualKey),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=%s", ts.URL, domainID, chanID, v-1, readers.GreaterThanKey),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=%s", ts.URL, domainID, chanID, v-1, readers.GreaterThanEqualKey),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=ab01", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?v=%f&comparator=wrong", ts.URL, domainID, chanID, v-1),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?vb=true", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?vb=yes", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?vs=%s", ts.URL, domainID, chanID, vs),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?vd=%s", ts.URL, domainID, chanID, vd),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?from=ABCD", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?to=ABCD", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?from=%f&to=%f", ts.URL, domainID, chanID, messages[19].Time, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?interval=10h", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10h", ts.URL, domainID, chanID),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10h&from=%f&to=%f", ts.URL, domainID, chanID, messages[19].Time, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusOK,
			res: pageRes{
		...
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=invalid&interval=10h&from=%f&to=%f", ts.URL, domainID, chanID, messages[19].Time, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10hrs&from=%f&to=%f", ts.URL, domainID, chanID, messages[19].Time, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10h&to=%f", ts.URL, domainID, chanID, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10h&to=ABCD&from=%f", ts.URL, domainID, chanID, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
		{
			...
			url:          fmt.Sprintf("%s/domains/%s/channels/%s/messages?aggregation=MAX&interval=10h&from=%f&to=ABCD", ts.URL, domainID, chanID, messages[4].Time),
			key:          thingToken,
			authResponse: true,
			status:       http.StatusBadRequest,
		},
