...
const (
	certsInstanceID  = "5de9b29a-feb9-11ed-be56-0242ac120002"
	certsContentType = "application/senml+json"
	serialNum        = "8e7a30c-bc9f-22de-ae67-1342bc139507"
	certsID          = "c333e6f-59bb-4c39-9e13-3a2766af8ba5"
	ttl              = "10h"
	commonName       = "test"
	token            = "token"
	agentToken       = "agent-token"
	certsDomainID    = "domain-certsID"
)
...
func TestIssueCert(t *testing.T) {
	...
	sdkConfig := sdk.Config{
		CertsURL:        ts.URL,
		MsgContentType:  certsContentType,
	...
	cases := []struct {
		...
		},
		{
			...
			svcresp:    certs.Certificate{},
			svcerr:     nil,
			err:        errors.NewSDKErrorWithStatus(errors.New("missing certsDomainID"), http.StatusBadRequest),
			domain:     "",
			token:      token,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			if tc.token == token {
				tc.session = smqauthn.Session{DomainUserID: certsID, UserID: certsID, DomainID: certsDomainID}
			}
...
func TestDeleteCert(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc:     "DeleteCert with empty entity certsID",
			entityID: "",
			svcerr:   certs.ErrMalformedEntity,
			err:      errors.NewSDKErrorWithStatus(certs.ErrMalformedEntity, http.StatusBadRequest),
			domain:   certsDomainID,
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			assert.Equal(t, tc.err, err)
			if tc.desc != "DeleteCert with empty entity certsID" && tc.desc != "DeleteCert with empty token" && tc.desc != "DeleteCert with empty domain" {
...
func TestListCerts(t *testing.T) {
	...
	cases := []struct {
		...
		{
			desc: "ListCerts success with entity certsID",
			sdkPm: sdk.PageMetadata{
				Offset:   0,
				Limit:    10,
				EntityID: certsID,
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err == nil {
				...
				if tc.desc == "ListCerts success with entity certsID" {
...
func TestIssueFromCSRInternal(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			agentSession := smqauthn.Session{DomainUserID: certsID, UserID: certsID, DomainID: certsDomainID}
